...
SCENE_TEMPLATES: tuple[SceneTemplate, ...] = (
    ...
    SceneTemplate(
        ...
        canonical_choice_texts={
            ...
            "letter_irrelevant": "그 애가 어떤 선택을 했든, 그건 이 계약과 무관한 일이오.",
            "letter_fold_silent": "(주먹을 말없이 움켜쥔다)",
            "ring_leah_gift": "이 반지는 총각 시절, 죽은 아내 리아에게 받은 것이오. 광야의 원숭이 떼를 다 준대도 나는 이걸 바꾸지 않았을 것이오.",
            "ring_loss_dignity": "내가 잃은 것을 아신다면, 당신은 이걸 약점이라 부르지 못할 것이오.",
            "ring_clutch_silent": "(빈 손가락을 조용히 감싸 쥔다)",
        },
    ),
    SceneTemplate(
    ...
    SceneTemplate(
        ...
        canonical_lines=(
            "샤일록, 정녕 그 살 한 조각이 당신에게 무슨 소용이 있소?",
            "먹을 수도, 팔 수도 없는 것을.",
            "이 법정도, 나 역시도 — 대체 무엇이 당신을 이토록 몰아붙이는지 이해할 수 없구려.",
            "......",
            "유대인은 눈이 없소?",
            "유대인은 손이, 오장육부가, 신체가, 감각이, 감정이, 열정이 없소?",
            "같은 음식을 먹고, 같은 무기에 상처 입고, 같은 병에 걸리고, 같은 약으로 낫고, "
            "같은 겨울에 춥고 같은 여름에 덥지 않소, 당신들 기독교인들처럼?",
            "찌르면 피 흘리지 않소? 간지럽히면 웃지 않소? 독을 먹이면 죽지 않소?",
            "그런데 해를 끼치면, 어찌 복수하지 않겠소?",
            "당신들이 나머지 모든 점에서 우리와 같다면, 이 점에서도 우리는 당신들과 같을 것이오.",
            "당신들이 나를 찔러도 나는 죽지 않겠소?",
            "당신들이 내게 잘못을 저질러도 나는 복수하지 않겠소?",
            "만일 우리가 나머지 모든 면에서 당신들과 같다면, 이 점에서도 우리는 당신들을 닮을 것이오.",
            "법정 안이 일순 조용해진다.",
            "야유하던 이들조차 잠시 입을 다문다.",
            "......",
            "...재판을 속개하겠소.",
            "줄곧 침착했던 안토니오의 손끝이 미세하게 떨린다.",
            "...그때, 저 사람은 아무 소리도 내지 않았었지.",
            "그는 다시 고개를 든다. 여전히 죽음을 받아들일 준비가 된 얼굴로.",
        ),
        canonical_line_kinds=(
            DialogueLineKind.SPEECH,
            DialogueLineKind.SPEECH,
