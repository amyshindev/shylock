...
### 1.1 목적
본 문서는 `shylock-trial.jsx` 데모(Anthropic Claude API 직접 호출 방식)를 실서비스
수준의 백엔드로 전환하기 위한 아키텍처 명세서다. 코드 구현은 다루지
않으며, 모듈 경계·책임·기술 스택·데이터 흐름만 정의한다.

**선행 조건 — 인증 문제 해결이 본 백엔드 도입의 1차 목적이다.**
`game-technical-spec.md` §4.1에서 이미 지적되었듯, 레거시 데모(`shylock-trial.jsx`)는 클라이언트 LLM 호출을 사용했으나
...
이어진다. 본 백엔드는 이 문제를 근본적으로 해결한다 — `ANTHROPIC_API_KEY`는
오직 6.1절의 outbound Adapter 내부(서버 환경변수)에만 존재하며,
프론트엔드는 Anthropic Claude API의 존재 자체를 모른 채 자체 API(8장)만 호출한다.
...
| `portia_response` | Anthropic Claude API를 호출해 포샤의 AI 반응·엔딩 서사를 생성하는 행위자 | Smith의 outbound 호출 부분(Claude 채팅) |
...
```text
        ...
        app/ports/output  (Port ABC)
                 │
    [ PostgreSQL / pgvector / Anthropic Claude API / Cohere ]  ← adapter/outbound
```
...
├── core/                                # DB 커넥션, Anthropic/Cohere 키 관리 등 공유 인프라
└── apps/
    └── shylock_trial/
        ├── adapter/
        │   ├── inbound/
        ...
        │       ├── client/
        │       │   ├── portia_response_client.py     # Anthropic Claude API 호출
...
TrialProgressionInteractor
  ├── PortiaResponseUseCase   → Anthropic Claude API 호출 (4.x 메서드에서 사용)
...
- `anthropic` SDK 사용, `claude-sonnet-4-6` 모델 고정
- **`ANTHROPIC_API_KEY`는 이 client 내부에서만 환경변수로 읽힌다.**
...
**Router가 하지 않는 일:** DB 쿼리, 임베딩 계산, Anthropic Claude API 호출.
...
| LLM | Anthropic Claude API (`claude-sonnet-4-6`), JSON structured output |
