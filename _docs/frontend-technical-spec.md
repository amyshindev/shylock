...
### 9.3 인증 — 프론트는 Anthropic API 키를 모른다

`backend-technical-spec.md` §1.1, 7.1절이 이미 해결한 문제를 프론트 쪽에서 재확인한다. **프론트엔드 코드 어디에도 `ANTHROPIC_API_KEY`나 Anthropic API 키가 등장하지 않는다.** `shylock-trial.jsx`의 레거시 LLM 직접 호출 패턴은 본 신규 구현에서 완전히 제거되며, 그 자리를 9.1절의 백엔드 API 클라이언트가 대신한다.
...
- [ ] 레거시 `shylock-trial.jsx`의 LLM API 직접 호출 패턴이 신규 코드 어디에도 재등장하지 않았는가(9.3절)
